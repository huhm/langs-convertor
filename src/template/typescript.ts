const TS_TEMPLATE_TYPE_NAME =
  "LANGINFO_{{=(it.moduleName||'ROOT').toUpperCase()}}"
const TS_TEMPLATE_MODULE_NAME = 'LangInfo{{!it.moduleName}}'
const TS_TEMPLATE = `/* eslint-disable prettier/prettier */
{{? it.langName.toLowerCase()!=='en'}}import type { ${TS_TEMPLATE_MODULE_NAME} } from './en'{{?}}
const ${TS_TEMPLATE_TYPE_NAME}{{? it.langName.toLowerCase()!=='en'}}:Partial<${TS_TEMPLATE_MODULE_NAME}>{{?}} = {{=it.moduleJsonString}}
